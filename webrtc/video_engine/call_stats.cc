...
int32_t CallStats::Process() {
  ...
  if (max_rtt > 0) {
    for (std::list<_StatsObserver*>::iterator it = observers_.begin();
...
void CallStats::RegisterStatsObserver(_StatsObserver* observer) {
  CriticalSectionScoped cs(crit_.get());
  for (std::list<_StatsObserver*>::iterator it = observers_.begin();
       it != observers_.end(); ++it) {
    if (*it == observer)
...
void CallStats::DeregisterStatsObserver(_StatsObserver* observer) {
  CriticalSectionScoped cs(crit_.get());
  for (std::list<_StatsObserver*>::iterator it = observers_.begin();
       it != observers_.end(); ++it) {
    if (*it == observer) {
